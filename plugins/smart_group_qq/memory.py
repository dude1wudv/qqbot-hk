...
class GroupMemory:
    ...
    def record(
        ...
        *,
        ...
        self.store.append_history(
            group_id,
            role="user",
            member_id=member_label(member_id),
            text=_redact(text),
    ...
    def _summary_source(self, group_id: str) -> tuple[str, int, dict[str, Any]]:
        state = self.store.memory_payload(group_id)
        after_id = int(state.get("last_history_id", 0))
        rows = self.store.get_history_since(group_id, after_id, limit=max(80, self.compact_after_messages * 4))
        if not rows:
    ...
    async def _refresh_ai_locked(self, ctx: Any, group_id: str, *, force: bool = False) -> dict[str, Any]:
        ...
        if not force and last_id <= int(state.get("last_history_id", 0)):
            return state.get("structured") or {}
    ...
    def background(self, group_id: str) -> str:
        ...
        recent = self.store.get_history_since(group_id, int(state.get("last_history_id", 0)), self.recent_context_messages)
        sections: list[str] = []
        if payload:
