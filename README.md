...
- 非 `@` 消息只在 QQ 开放平台实际投递 `GROUP_MESSAGE_CREATE` 时进入本群 ambient 历史和摘要链路。`ambient.participation` 启用后，高置信度的求助、有效问题及请求回应/测试收发/确认收到进入原生群会话，不要求完整句式或再次点名机器人；闲聊、仅转述测试结果、斜杠管理命令、超过 120 秒的排队消息、分类失败均保持安静。每群默认 5 秒参与冷却；官方 `@机器人` 不受该冷却限制，并取消尚未完成的旧参与判断。`@` 其他成员保持静默，不解释“不插嘴”。定时任务不能补拉平台没有投递的历史消息。
