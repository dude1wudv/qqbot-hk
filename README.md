...
- 回复默认采用 QQ 群友风格：群聊通常 1～2 句、总共约 80 字，不以“你呢”“要不要我”等反问强行续聊；常见模板式结尾还有发送前过滤。群聊模型正文按句子/换行分条，常规每条不超过 120 字；主动插话最多 180 字、3 条。被点名的长答案和代码保留完整内容，按长文本分段。私聊通常约 300 字。常见 Markdown 统一降级为纯文本。分条沿用原消息锚点，失败停止，重试跳过已成功分条；完整发送后只登记一轮回复及其各条消息引用。
...
- 有内容的陈述和同话题的其他成员消息也会进入参与判断；话题窗口为 120 秒。`ambient.participation.max_interjections_per_minute: 2` 限制每群一分钟内最多放行两轮非点名参与（生成中或最终选择不回复也占额度，防止并发刷屏）。连续两轮成功发言未得到回应后，按 `unanswered_pause_seconds: 300` 暂停新话题插话；窗口内明确回应机器人的同话题消息可解除退场，但仍受分钟额度限制。叫名字、官方 @、引用机器人的消息不受主动插话额度限制。限频状态保存在进程内，重启后重新计数；不会定时冷场发话。
...
部署后验证任意已加入的新群通过适配器与中央群授权，未批准私聊仍被拒绝或进入 pairing，两名成员共享本群上下文且不串群。群管理员打开“接收所有消息”并确认平台实际投递后，再验证：有内容的分享按需接话、纯表情/收尾静默、多人延续话题、主动插话限频与退场、短句分条、重复事件只处理一次、非 @ 管理命令不执行、近期旁听能作为后续 @ 的上下文。主动推送还要求群内允许主动发送，不能用修改本地配置绕过平台权限。
