...
- 角色采用自然群友风格，可以主动提问和完整表达；不再机械过滤末尾问句，也不再截断主动回复为 180 字。短回复按句子/换行分条，长回复与代码完整保留，以每包最多 1500 字进行传输分段。命令回复也不再最多发送五段。常见 Markdown 统一降级为纯文本。分条沿用原消息锚点，失败停止，重试跳过已成功分条；完整发送后只登记一轮回复及其各条消息引用。
