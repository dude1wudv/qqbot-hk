...
class HermesChatReasoningPatchTests(unittest.TestCase):
    ...
    def test_executed_patch_preserves_deepseek_and_mimo_effort_and_route_boundaries(self):
        ...
        for model in ("deepseek/deepseek-v4.1-flash", "DEEPSEEK/DEEPSEEK-V4.1-FLASH"):
            for enabled in (True, False):
                for supports in (True, False):
                    with self.subTest(route="deepseek", model=model, enabled=enabled, supports=supports):
                        ...
                        if enabled:
                            effort.clamp_effort.assert_called_once_with(
                                "xhigh", effort.DEEPSEEK_V4_EFFORTS, effort.DEEPSEEK_V4_OVERRIDES
                            )
                        else:
        ...
        for enabled in (True, False):
            for supports in (True, False):
                with self.subTest(route="mimo", enabled=enabled, supports=supports):
                    effort.clamp_effort.reset_mock()
                    kwargs = {"untouched": "Value"}
                    config = {"enabled": enabled, "effort": "xhigh"}
                    namespace["build"](
        ...
        for host, model in (
            ("https://other.example/v1", mimo_model),
            ("http://sub2api:8080/v1.evil", mimo_model),
            (PATCH.SUB2API_BASE_URL, "xiaomi/mimo-v2.6-flash-preview"),
            (PATCH.SUB2API_BASE_URL, "Muse/muse-chat"),
        ):
