...
class FormatterTests(unittest.TestCase):
    ...
    def test_envelope_shaped_plain_text_is_not_parsed(self):
        for text in (
            '{"action":"ignore","message":null}',
            '{"action":"reply","message":"literal answer"}',
        ):
            with self.subTest(text=text):
                self.assertEqual(format_for_qq(text), text)
                self.assertEqual(split_group_reply(text), [text])
                self.assertEqual(split_group_reply(text, direct=True), [text])

    def test_markdown_is_rendered_as_plain_text(self):
