...
5. 固定 Hermes digest 中，QQ STT 的 `baseUrl`/`model` 应放在 `platforms.qqbot.extra.stt`；但该版本只有在同一 YAML 块内也写入 `apiKey` 时才采用这些字段，未实现从 `QQ_STT_API_KEY` 的密钥回退。为避免把秘密写入 YAML，派生镜像必须最小修补并在镜像内做行为验证。
6. 固定 Hermes digest 的通用媒体交付调用 `send_voice` 时可能不传 `reply_to`；QQ `_send_media` 此时不会把被动回复所需的 `msg_id` 放进消息体。派生补丁必须用当前 chat 最近的入站消息 ID 补齐锚点，并验证显式 `reply_to` 仍优先。
7. 阿里 Qwen 音频不是普通 Chat Completions：ASR 使用 DashScope multimodal-generation HTTP；TTS Plus 使用 DashScope `api-ws/v1/inference` 的 `run-task/continue-task/finish-task` 协议。不能把现有 `/v1/chat/completions` 直接改路径转发。
8. 生产账号 970 的模型映射和分组 81 已包含三个 Qwen 音频模型；实现必须仍按 group/model 调度，不能硬编码账号 970。分组 81 当前音频单价为空，会落到 Sub2API 内置默认价；上线前必须写入供应商实际 STT/TTS 单价，否则账务不准确。

## 本轮部署前执行状态

- 阶段 1、阶段 2 的核心代码与首批测试已写入各自独立仓库的工作区；阶段 3 尚未开始。
- QQ 侧固定 digest 派生镜像已构建，真实 adapter 行为 smoke 通过，全量 66 项测试通过。
- Sub2API 侧两个音频路由、Qwen HTTP/WebSocket 传输、OpenAI 错误封装和 usage 计费已实现；独立复审要求的边界修订与相关 Go 测试已经通过。
- Sub2API 在进入阶段 3 前仍须补齐路由级 auth/group/platform、真实多账号 failover、usage 持久化/去重和超限 multipart 的端到端断言；当前通过的是相关包与协议单元测试，不能替代这些集成验证。
- 以下状态记录的是进入阶段 3 前的发布基线；是否已提交、推送或上线必须以本轮最终 Git SHA 与 HK 运行时核验为准。
...
在 `sub2api-hk-src/backend/internal/server/routes/gateway.go` 注册以下受现有 API key、group assignment、request ID、错误保护和 body limit 中间件保护的路由。音频路由必须使用 OpenAI 格式的 group-assignment/error middleware，不能继承现有 `/v1` group 的 Anthropic 错误封装；所有参数错误返回 OpenAI `invalid_request_error`：
...
- `POST /v1/audio/speech`
  ...
  - 产品级 `input` 上限固定为 4,000 个 Unicode code point，在调度和上游连接前拒绝；这严于供应商单次 `continue-task` 20,000 字符、单任务累计 200,000 字符上限，避免 QQ 超长语音与意外费用。
...
- 从已调度账号取得 API key，并读取账号非秘密配置 `extra.qwen_audio_http_base_url` 与 `extra.qwen_audio_ws_base_url`；两者是地域/workspace API 根地址，由服务端安全追加固定 ASR/TTS 路径。只接受 HTTPS/WSS，host 必须属于配置账户所在地域/workspace 的 DashScope 域名白名单，HTTP/WSS 根地址的地域必须一致，禁止请求体控制或拼接上游 host，避免双拼路径、SSRF 和跨地域 key 失配。
...
- 把 OpenAI `instructions` 映射为 Qwen 的单数 `instruction`；Qwen Audio TTS 不套用 CosyVoice 的 100 计数单位限制，只执行供应商对目标模型公开的长度/内容约束。把 OpenAI `speed` 校验后映射为 Qwen `rate`。越界参数直接拒绝，不静默截断或钳位。
...
- TTS：只采用成功的最终 `task-finished.payload.usage.characters / 1_000_000`，模式 `tts`；该字段缺失、非整数、负数或超出请求上限时整次请求失败并告警，不用本地 RuneCount 估算，也不能把多个 `sentence-end` 的累计值再次相加。
...
- `backend/internal/handler/qwen_audio_billing_test.go`：最终 `task-finished` 中/英文及混合文本 `usage.characters`、字段缺失/非法失败、多个 `sentence-end` 不重复累加、WAV 精确时长、重复 client request ID 不合并两笔真实音频请求。
...
- `qqbot-hk/scripts/patch-hermes-audio.py`：对 pinned adapter 的预期片段和文件 SHA 做 fail-closed 校验后，完成四项改动：
  ...
  3. `platforms.qqbot.extra.stt` 未写 `apiKey` 时只把 `QQ_STT_API_KEY` 用作密钥回退，仍采用 YAML 中的 `baseUrl` 与 `model`。
  4. `send_voice` 未收到显式 `reply_to` 时，使用该 chat 最近的入站 `message_id` 作为 QQ 被动消息锚点；显式锚点必须优先。这样不复制整个 adapter，也不修改通用媒体路由。
...
`QQ_STT_BASE_URL` 与 `QQ_STT_MODEL` 不再作为部署配置来源；固定 digest 的环境变量整段回退行为容易让 YAML 看似生效但实际被忽略。镜像 smoke 必须断言“YAML baseUrl/model + 环境变量密钥”解析出的最终三元组精确匹配。
...
- `qqbot-hk/tests/test_hermes_audio_patch.py`：固定输入片段、重复执行幂等、错误 SHA/缺失哨兵 fail-closed；验证关闭 built-in ASR 后仍调用外部 STT、YAML STT 地址/模型配合环境变量密钥生效、voice 事件类型不丢失、MP3 走 `MEDIA_TYPE_VOICE` 且群媒体带被动回复锚点。
- 扩展 `qqbot-hk/tests/test_smart_group_qq.py`：语音文本仍经过当前群 ACL、审核、记忆与 session 隔离；私聊不被群插件改写。
- 镜像 smoke：构建派生镜像后导入真实 `QQAdapter`，用 fake HTTP/发送方法验证 `QQ_STT_PREFER_BUILTIN=false`、YAML STT 实际解析键、`MessageType.VOICE`、`auto_tts`、`tts` tool、MP3 原生语音 `file_type=3` 与群媒体 reply anchor，不测试复制的伪接口。
- 配置测试：QQ 工具集含 `tts` 但仍不含 terminal/file/code execution；TTS provider、模型、base URL 与 voice 精确匹配；秘密只存在部署态 `.env`。

本地验证门（本机直接运行完整 unittest 需要 `tzdata` 与 `PyYAML`；若宿主未安装，必须在固定派生镜像内运行，不能把 import error 误报为测试通过）：

```bash
cd qqbot-hk
docker compose build hermes-qqbot
docker run --rm --entrypoint python --workdir /workspace -v "$PWD:/workspace:ro" qqbot-hk/hermes-agent:v0.21.0-audio-v1 -m unittest discover -s tests -v
docker run --rm --entrypoint python qqbot-hk/hermes-agent:v0.21.0-audio-v1 /opt/hermes/verify-hermes-audio.py
```
...
4. 在 `qqbot-hk` 提交、经代理推送精确 SHA；HK 构建固定 digest 的派生镜像，备份 Compose 与 `/opt/qqbot-hk-deploy/hermes-data/{config.yaml,SOUL.md,plugins,cron}` 后只重建 `hermes-qqbot`。
5. 运行扩展后的 `scripts/verify-server.sh`：检查派生镜像标签/基础摘要、补丁标记、三个语音秘密/行为环境变量存在性、STT YAML 实际解析结果、TTS 配置、`tts` 工具集、MP3 原生语音/reply anchor 行为、Sub2API 两个音频端点可达、容器健康；只输出布尔值和计数。
...
7. 观察至少一个 QQ WebSocket reconnect 周期，确认重连后 ASR/TTS、生产 Gateway 的开发体验用户私聊边界、群 ACL 与工具限制仍有效；不得重新启用旧 `QQ_SANDBOX` 路由。
...
4. `qqbot-hk/scripts/patch-hermes-audio.py` — 仅 Qwen ASR、YAML 密钥回退、VOICE 类型和 QQ 媒体 reply anchor 的最小 pinned-source 补丁。
